...
The single-product Amazon worker accepts either `/dp/ASIN` or
`/gp/product/ASIN` URLs. It reads the active Buy Box first, then See All Buying
Options or the offer-listing page when needed. A candidate must match the
expected ASIN, be both sold by and shipped from Amazon, expose an enabled
purchase control and current offer token, and stay within the configured item
price limit.
...
The repository-level `/amazon-buy <product-url>` skill is the operator entry
point for this single-product flow. Invocation authorizes one quantity-one
order, uses the user-approved `$10000` item and order-total fail-safe ceilings,
ignores return-policy text, refuses third-party offers, and does not expose the
generated checkout URL. Its launcher reuses the authenticated CDP endpoint
