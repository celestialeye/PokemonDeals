...
`/amazon-buy <Amazon product URL>`. That invocation authorizes exactly one
quantity-one Amazon order and launches the single-product direct-buy worker with
the documented `$10000` fail-safe ceilings.
...
- **Amazon direct buy (`amazon-preorder.js`):** The flow accepts a `/dp/ASIN` or `/gp/product/ASIN` URL, validates the expected ASIN/title, and monitors for an Amazon-shipped and Amazon-sold offer under the configured item-price limit. It extracts the current offer token and enters Buy Now checkout without adding to cart, refreshes unavailable checkout states while periodically reacquiring rotated tokens, enforces an order-total limit, clicks Place order at most once, and stops only on explicit confirmation. `amazon:preorder` remains an alias for `amazon:direct-buy`.
